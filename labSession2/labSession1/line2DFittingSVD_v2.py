...
# ---- 3.1: Fit line using only extreme points ----
# Select the first and last noisy points
x_extreme = x[:, [0, -1]]          # keep only 2 points
u, s, vh = np.linalg.svd(x_extreme.T)
l_extreme = vh[-1, :]              # line through the 2 extreme points
drawLine(l_extreme, 'm--', 1.5)    # magenta dashed line
print("Line from extreme points:", l_extreme)


# ---- 3.2: Fit line using ground-truth (xGT) points ----
u, s, vh = np.linalg.svd(xGT.T)
l_GT_est = vh[-1, :]
drawLine(l_GT_est, 'c--', 1.5)     # cyan dashed line
print("Line from noiseless points:", l_GT_est)